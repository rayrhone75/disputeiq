...
export function EscalationPanel({
  ...
  return (
    <div className="rounded-2xl border border-border-strong bg-surface p-6 space-y-5">
      <div>
        <h2 className="text-lg font-semibold text-fg">Escalation options</h2>
        ...
        {STAGES.map((s) => {
          ...
          return (
            ...
            >
              ...
              <p className="mt-2 text-xs text-fg-muted">{s.desc}</p>
              <p className="mt-1 text-[10px] text-fg-subtle">{s.legal}</p>
              <button
      ...
      {preview && (
        <div className="rounded-xl border border-border-strong bg-surface-muted/60 p-5 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-sm font-semibold text-fg">
                {STAGES.find((s) => s.key === preview.stage)?.label} — draft ready
              </div>
              <div className="text-[10px] text-fg-subtle">{preview.legalBasis}</div>
            </div>
          ...
          <div className="relative">
            ...
            <pre className="max-h-64 overflow-y-auto whitespace-pre-wrap rounded-lg bg-surface p-4 text-xs text-fg select-none ring-1 ring-border-strong">
              {preview.bodyText}
            </pre>
          </div>
          ...
          <div className="flex gap-3">
            ...
            <button
              ...
              className="rounded-lg bg-surface px-4 py-2 text-xs text-fg-muted ring-1 ring-border-strong"
            >
              Cancel
            </button>
          </div>
