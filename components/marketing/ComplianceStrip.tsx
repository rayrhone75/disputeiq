...
export function ComplianceStrip() {
  return (
    <section className="relative border-t border-border bg-canvas">
      <div className="mx-auto max-w-7xl px-6 py-14">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {pillars.map((p) => (
            <div
              key={p.title}
              className="relative rounded-2xl border border-border-strong bg-surface/80 p-6"
            >
              <span className="absolute right-5 top-5 h-1.5 w-1.5 rounded-full bg-indigo-500/70" />
              <h3 className="font-serif text-[17px] leading-tight tracking-tight text-fg">
                {p.title}
              </h3>
              <p className="mt-3 text-[12.5px] leading-relaxed text-fg-muted">{p.body}</p>
            </div>
          ))}
        </div>

        <p className="mt-8 text-[11px] leading-relaxed text-fg-subtle">
